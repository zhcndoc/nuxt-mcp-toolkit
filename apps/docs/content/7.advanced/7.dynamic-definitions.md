  ...
  handler: async () => {
    ...
    return {
      messages: [{
        ...
        content: {
          type: 'text',
          text: `欢迎 ${event.context.user.name}！以下是开始使用的方法...`,
        },
      }],
    }
  },
...
export default defineMcpTool({
  description: '为特定查询创建一个快捷工具',
  inputSchema: {
    name: z.string().describe('快捷工具名称'),
    query: z.string().describe('此快捷方式执行的查询'),
  },
  handler: async ({ name, query }) => {
    const mcp = useMcpServer()
    mcp.registerTool(name, {
      description: `快捷方式：${query}`,
    }, async () => {
      const result = await runQuery(query)
      return { content: [{ type: 'text', text: JSON.stringify(result) }] }
    })
    return `快捷工具 "${name}" 已创建`
  },
})
```
...
export const registerAgent = defineMcpTool({
  ...
  handler: async ({ agentId, agentName }) => {
    ...
    mcp.registerTool(`call-${agentId}`, {
      description: `调用代理：${agentName}`,
    }, async () => {
      const response = await callAgent(agentId)
      return { content: [{ type: 'text', text: response }] }
    })
    return `代理 "${agentName}" 已注册`
  },
})
...
export const unregisterAgent = defineMcpTool({
  ...
  handler: async ({ agentId }) => {
    ...
    if (!removed) throw createError({ statusCode: 404, message: `未找到代理 "${agentId}"` })
    return `代理 "${agentName}" 已取消注册`
  },
})
```
...
- [Middleware](/advanced/middleware) — 设置身份验证上下文
- [Sessions](/advanced/sessions) — 启用每个会话的状态
