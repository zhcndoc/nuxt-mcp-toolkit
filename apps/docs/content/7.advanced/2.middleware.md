...
对于大多数情况，你只需要在处理器运行之前设置上下文。采用**软**处理方式——仅在认证成功时设置上下文，并允许未认证请求继续：
...
**不要在 MCP 中间件中抛出 `401`。** 许多 MCP 客户端会将 401 视为启动 OAuth 发现（查找 `.well-known/oauth-*` 端点）的信号。请以“软”方式设置上下文，并使用[`enabled` 守卫](/advanced/dynamic-definitions)或按工具检查来控制功能。完整模式请参见[身份验证](/examples/authentication)。
::
...
当令牌有效时，将用户设置到上下文中；否则不执行任何操作。需要身份验证的工具可以检查 `event.context.user`，并使用[`enabled` 守卫](/advanced/dynamic-definitions)对匿名调用者隐藏自身：
...
请参阅[身份验证指南](/examples/authentication)获取完整演练（Better Auth API 密钥、自定义验证、客户端配置）。

## 日志和分析示例

::callout{icon="i-lucide-arrow-right" color="primary" to="/advanced/logging"}
对于结构化的广泛事件、汇聚器（Axiom、Sentry、OTLP、Datadog 等）以及按工具的上下文，请使用[`useMcpLogger()`](/advanced/logging)而不是下面的手动 `console.log` 模式。此处的中间件示例保持最小化，因此它不依赖 `evlog`。
::
...
::callout{icon="i-lucide-info" color="info"}
对于特定的进行中工具调用，抛出 `403` 是安全的——当 `tools/call` 到达时，客户端已经完成初始化，不会进入 OAuth 发现流程。“不抛出”规则适用于**在传输层缺少/无效认证时抛出 `401`**。
::
