...
- [Sessions](/advanced/sessions) — 在后续工具调用之间存储用户的启发式询问回答。
- [Middleware](/advanced/middleware) — 根据上下文控制哪些工具需要启发式询问。
- [Dynamic definitions](/advanced/dynamic-definitions) — 使用 `enabled` 守卫隐藏工具，而不是交互式询问。
- [Logging](/advanced/logging) — 在宽事件上记录启发式询问结果（`accept` / `decline` / `cancel`）。
