---
title: Request input from users with elicitation
...
::prompt
---
description: Add MCP elicitation (form & URL)
icon: i-lucide-message-square-quote
actions:
  - copy
  - cursor
...
- [Sessions](/advanced/sessions) — store the user's elicitation answers across subsequent tool calls.
- [Middleware](/advanced/middleware) — gate which tools require elicitation based on context.
- [Dynamic definitions](/advanced/dynamic-definitions) — hide tools behind `enabled` guards instead of asking interactively.
- [Logging](/advanced/logging) — record elicitation outcomes (`accept` / `decline` / `cancel`) on the wide event.
