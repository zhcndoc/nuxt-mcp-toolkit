...
| Helper | Returns |
| --- | --- |
| `listMcpTools(options?)` | `McpToolSummary[]` |
| `listMcpResources(options?)` | `McpResourceSummary[]` |
| `listMcpPrompts(options?)` | `McpPromptSummary[]` |
| `listMcpDefinitions(options?)` | `{ tools, resources, prompts }` |

Each summary contains `name`, `title`, `description`, `group`, and `tags` (plus `uri` for resources). Names auto-generated from the filename are already resolved, so what you get matches exactly what the MCP client sees in `tools/list`.
...
Filters compose freely — request-aware visibility plus group and tag narrowing in one call:
