...
- Optional: install evlog and register `evlog/nuxt` for wide events — `pnpm add evlog` (or npm/yarn/bun) and add `'evlog/nuxt'` to `nuxt.config.ts > modules`
- Configure from the top-level `evlog: { … }` key (service, sampling, redact, routes, exclude, …)
- Tag MCP traffic with a dedicated service via `evlog.routes['/mcp/**'] = { service: 'mcp' }`
...
`log.notify(...)` works out of the box — no extra setup needed. Server-side wide events are powered by [evlog](https://evlog.dev): install it, register the `evlog/nuxt` module, and configure everything from the top-level `evlog: { … }` key in `nuxt.config.ts`.

### Enable wide events

Install evlog and register `evlog/nuxt`:
...
export default defineNuxtConfig({
  ...
  evlog: {
    env: { service: 'my-app' },
    // Configure drains, sampling, redaction, exclude, …
  },
})
```

That's it. Wide events show up in your dev terminal on every MCP request, with `mcp.tool`, `mcp.session_id`, and friends already tagged on top of whatever the `evlog: { … }` config sets.

### Tag MCP traffic with a dedicated service

Use `evlog.routes` to give MCP requests their own `service` field while leaving the rest of your app untouched:
...
`useMcpLogger()` requires `nitro.experimental.asyncContext` to be `true` (default since Nuxt 3.8+). The `set`, `event`, and `evlog` channels throw an `McpObservabilityNotEnabledError` when `evlog/nuxt` is not registered. The `notify` channel keeps working in both cases.
::
