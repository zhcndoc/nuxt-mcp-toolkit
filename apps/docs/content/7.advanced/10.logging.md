---
title: Capture MCP logs and wide events
...
- [Middleware](/advanced/middleware) — capture `requestId`, timing, and tool names in your handler middleware.
- [Sessions](/advanced/sessions) — tag every wide event with `session.id` automatically.
- [Authentication](/examples/authentication) — soft auth populates `user.id` / `user.email` on every event.
- [evlog docs](https://evlog.dev) — drains, sampling, redaction, and custom adapters.
