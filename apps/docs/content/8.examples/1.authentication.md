...
1. **使用 `enabled` 隐藏工具** —— 需要身份验证的工具不会出现在匿名调用者的 `tools/list` 中（推荐）。
2. **返回对用户友好的消息** —— 工具仍然可见，但在没有用户上下文时会说明如何进行身份验证。避免在工具处理程序中抛出 `401`，以免 MCP 客户端进入 OAuth 发现。
...
::callout{icon="i-lucide-info" color="info"}
`enabled` 守卫在中间件之后运行，因此它可以看到上面设置的用户上下文。更多模式请参见 [Dynamic Definitions](/advanced/dynamic-definitions)。
::

::callout{icon="i-lucide-info" color="info"}
请记得在 Nuxt 配置中启用 `asyncContext` 以使用 `useEvent()`：
