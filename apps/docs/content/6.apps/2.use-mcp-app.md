...
```ts
const {
  initialData,  // Ref<T | null>          — 挂载时处理程序有效载荷的快照，此后不会更新
  data,         // Ref<T | null>          — 从 structuredContent 水合，通过 callTool 刷新
  loading,      // Ref<boolean>           — 在收到第一个有效载荷前为 true
  ...
  sendPrompt,   // (prompt: string) => void              — 将消息推送到聊天中
  openLink,     // (url: string) => void                 — 请求主机打开 URL
} = useMcpApp<MyPayload>()
```
...
`initialData` 是处理程序的 `structuredContent` 在构建时内联的**冻结快照**（在 ChatGPT 中则是 `window.openai.toolOutput`）。它永远不会改变——可将它用于 ID 或查询参数等引导值，因为后续的 `callTool` 结果可能会覆盖 `data` 中的这些值。

当处理程序返回 `structuredContent` 时，`data` 会在首次渲染时**直接填充**，随后由 `callTool` 或主机推送的 `tool-result` 刷新。`loading` 初始为 `true`，在收到第一个有效载荷后变为 `false`。对于正在执行的 `callTool()` 刷新操作，请使用 `pending`：
...
async function refresh() {
  ...
  // `data` 现在保存最新的工具结果；`initialData` 仍然包含 `{ listId }`
}
</script>
```
