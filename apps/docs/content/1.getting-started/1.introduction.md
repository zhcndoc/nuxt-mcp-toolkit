...
MCP 应用是交互式 UI 小部件，像 Cursor 和 ChatGPT 这样的宿主会以内联方式渲染它们。将其作为 `app/mcp/` 中的 Vue 单文件组件进行编写——该工具包会将它们打包并作为 MCP UI 资源提供。
...
- [工具指南](/tools/overview) - 创建您的第一个 MCP 工具
- [应用指南](/apps/overview) - 为 AI 宿主构建交互式 UI 小部件
