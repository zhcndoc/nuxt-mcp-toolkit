---
title: 安装 Nuxt MCP 工具包
description: 在你的项目中安装并配置 Nuxt MCP 模块。
navigation:
  title: 安装
  icon: i-lucide-download
seo:
  title: 安装 Nuxt MCP 工具包
  description: 在任何 Nuxt 3 或 4 应用中安装 @nuxtjs/mcp-toolkit —— 自动从 server/mcp/ 中发现工具、资源和提示词，并在 /mcp 提供它们。
---
...
- 使用 `npx nuxt add mcp` 自动安装该模块
- 创建 `server/mcp/` 目录，以及 `tools/`、`resources/` 和 `prompts/` 子目录
- `defineMcpTool`、`defineMcpResource`、`defineMcpPrompt` 和 `defineMcpHandler` 会自动导入
- 使用 `defineMcpTool` 和 Zod 的 `inputSchema` 在 `server/mcp/tools/test.ts` 中创建一个测试工具
- 启动开发服务器并验证 MCP 端点 `http://localhost:3000/mcp`
- 将你的 IDE（Cursor 或 VS Code）连接到 `http://localhost:3000/mcp`
...
- [配置](/getting-started/configuration) - 了解所有配置选项
- [连接](/getting-started/connection) - 将 AI 助手连接到您的 MCP 服务器并为您的文档添加安装按钮
- [工具](/tools/overview) - 创建您的第一个工具
